...
from utils.utils import fix_random_seed, cal_acc_comb, data_loader, cal_bca_comb, cal_acc_online, TTA_ensemble, TTA_ensemble_delay
...
def train_target(args):
    ...
    if args.paradigm == 'MI':
        acc_t_te = TTA_ensemble(dset_loaders["Target-Online"], base_networks, args)
        #acc_t_te = TTA_ensemble_delay(dset_loaders["Target-Online"], base_networks, args)
        log_str = 'Task: {}, TTA Acc = {:.2f}%'.format(args.task_str, acc_t_te)
        args.log.record(log_str)
        print(log_str)
    elif args.paradigm == 'ERP':
        acc_t_te = TTA_ensemble(dset_loaders["Target-Online"], base_networks, args)
        #acc_t_te = TTA_ensemble_delay(dset_loaders["Target-Online"], base_networks, args, balanced=False)
