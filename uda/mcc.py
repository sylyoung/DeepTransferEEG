...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.paradigm == 'MI':
                #acc_t_te, y_pred = cal_acc_comb(dset_loaders["Target"], base_network)
                #acc_t_te, y_pred = cal_acc_comb(dset_loaders["Target-Imbalanced"], base_network)
                acc_t_te, y_pred = cal_auc_comb(dset_loaders["Target-Imbalanced"], base_network)
                ...
                #acc_t_te, y_pred = cal_bca_comb(dset_loaders["Target"], base_network)
                acc_t_te, y_pred = cal_bca_comb(dset_loaders["Target-Imbalanced"], base_network)
            ...
            base_network.train()
    #torch.save(netF, './runs/' + str(args.data) + '/EA_MCC_' + 'netF' + '_test_subj_' + str(args.idt) + '_seed_' + str(args.SEED) + '.ckpt')
    #torch.save(netC, './runs/' + str(args.data) + '/EA_MCC_' + 'netC' + '_test_subj_' + str(args.idt) + '_seed_' + str(args.SEED) + '.ckpt')
