...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        discrepancy_loss = lmmd(features_source, features_target, labels_source, outputs_target, class_num=class_num)
        args.discrepancy_weight = 2 / (1 + math.exp((-10 * iter_num / max_iter)) / len(dset_loaders["source"])) - 1
        #print(np.round(classifier_loss.item(), 3), np.round(discrepancy_loss.item(), 3), np.round(args.discrepancy_weight, 3))
        total_loss = classifier_loss + args.discrepancy_weight * discrepancy_loss
