...
class Deformer(nn.Module):
    ...
    def __init__(self, *, num_chan, num_time, temporal_kernel, num_classes, num_kernel=64,
                 depth=4, heads=16,
...
class Deformer_CLIP(nn.Module):
    def cnn_block(self, out_chan, kernel_size, num_chan):
        return nn.Sequential(
            nn.Conv2d(1, out_chan, kernel_size, padding=(0, 1)),  # slightly padded temporal conv
            nn.Conv2d(out_chan, out_chan, (num_chan, 1), padding=0),  # collapses spatial/channel dim
            nn.BatchNorm2d(out_chan),
            nn.ELU(),
            nn.AdaptiveAvgPool2d((1, 64))  # ⬅️ force temporal dim to 64 exactly
        )

    def __init__(self, *, num_chan, num_time, temporal_kernel, num_kernel=64,
                 depth=4, heads=16,
                 mlp_dim=16, dim_head=16, dropout=0.5, use_fc=False):
        super().__init__()

        self.cnn_encoder = self.cnn_block(out_chan=num_kernel, kernel_size=(1, temporal_kernel), num_chan=num_chan)

        dim = 64  # Fixed output temporal dim from CNN
        self.to_patch_embedding = Rearrange('b k c f -> b k (c f)')  # becomes b × num_kernel × 64
    ...
    def forward(self, eeg):
        x = self.cnn_encoder(eeg)  # (B, num_kernel, 1, 64)
        x = self.to_patch_embedding(x)  # (B, num_kernel, 64)
        x += self.pos_embedding
        x = self.transformer(x)
        if self.use_fc:
            x = self.fc_layer(x)
...
class Regression_head(nn.Module):
    def __init__(self, inputs_dim):
        ...
        self.regression_head = nn.Sequential(
            nn.Linear(in_features=self.inputs_dim, out_features=1,))
...
if __name__ == "__main__":
    batch_size = 128
    num_channels = 30  # 64
    num_timesamples = 750  # 126
    data = torch.ones((batch_size, 1, num_channels, num_timesamples))
    model = Deformer_CLIP(num_chan=num_channels, num_time=num_timesamples, temporal_kernel=11, num_kernel=64,
                          depth=4, heads=16, mlp_dim=16, dim_head=16, dropout=0.5)
    print(model)
    print(count_parameters(model))  # 1101680

    out = model(data)
    print(out.shape)    # torch.Size([128, 512])